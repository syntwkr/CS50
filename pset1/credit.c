...
int main(void)
{
    ...
    if (total_sum % 10 == 0)
    {
        ...
        if (digits == 15)
        {
            if ((orig_number / (long) 10E12) == 34 ||
                (orig_number / (long) 10E12) == 37)
            {
                card_type = "AMEX";
            }
            else
        ...
        {
            // Check Mastercard
            if ((orig_number / (long) 10E13) >= 51 &&
                (orig_number / (long) 10E13) <= 55)
            {
                card_type = "MASTERCARD";
            }
            // Check some Visas
            else if ((orig_number / (long) 10E14) == 4)
            {
                card_type = "VISA";
            }
            else
